...
export default function WeeklyReviewSettingsPage() {
  ...
  const isTeamSharingEnabled = (teamId: string) => {
    return sharingSettings?.some((setting: any) => 
      setting.teamId === teamId && setting.isEnabled
    ) ?? false;
  };

  const enabledTeamsCount = sharingSettings?.filter((setting: any) => setting.isEnabled).length ?? 0;
  const availableTeamsCount = organizationTeams?.length ?? 0;

  if (loadingSettings || loadingOrganizationTeams) {
    return (
      ...
      <Stack gap="xl">
        ...
        <div>
          <Title order={1} className="text-text-primary">
            Weekly Review Sharing Settings
          </Title>
          <Text c="dimmed" mt="xs">
