"use client";

import { Menu, Avatar, Text, UnstyledButton } from "@mantine/core";
import { useMantineColorScheme } from "@mantine/core";
import {
  IconSettings,
  IconMessageReport,
  IconSun,
  IconMoon,
  IconLogout,
  IconChevronUp,
} from "@tabler/icons-react";
import Link from "next/link";
import { useState } from "react";
import { SidebarFeedback } from "./SidebarFeedback";

interface UserMenuProps {
  session: {
    user: {
      name?: string | null;
      email?: string | null;
      image?: string | null;
    };
  };
  onClose: () => void;
}

export function UserMenu({ session, onClose }: UserMenuProps) {
  const { colorScheme, toggleColorScheme } = useMantineColorScheme();
  const [feedbackOpen, setFeedbackOpen] = useState(false);

  const user = session.user;
  const initials = user.name
    ? user.name
        .split(" ")
        .map((n) => n[0])
        .join("")
        .toUpperCase()
        .slice(0, 2)
    : "U";

  const handleThemeToggle = () => {
    const newScheme = colorScheme === "dark" ? "light" : "dark";
    toggleColorScheme();
    const html = document.documentElement;
    if (newScheme === "dark") {
      html.classList.add("dark");
    } else {
      html.classList.remove("dark");
    }
    localStorage.setItem("color-scheme", newScheme);
  };

  return (
    <>
      <div className="flex-shrink-0 border-t border-border-primary px-3 py-3">
        <Menu shadow="md" width={240} position="top-start" offset={8}>
          <Menu.Target>
            <UnstyledButton className="flex w-full items-center gap-3 rounded-lg px-2 py-2 hover:bg-surface-hover transition-colors">
              <Avatar
                src={user.image}
                alt={user.name ?? "User"}
                size="sm"
                radius="xl"
                className="bg-brand-primary text-white"
              >
                {initials}
              </Avatar>
              <Text
                size="sm"
                fw={500}
                truncate
                className="flex-1 text-text-primary"
              >
                {user.name ?? "User"}
              </Text>
              <IconChevronUp size={14} className="text-text-muted" />
            </UnstyledButton>
          </Menu.Target>

          <Menu.Dropdown className="bg-surface-secondary border-border-primary">
            {user.email && (
              <>
                <Menu.Label className="text-text-muted">
                  {user.email}
                </Menu.Label>
                <Menu.Divider className="border-border-primary" />
              </>
            )}

            <Menu.Item
              leftSection={<IconSettings size={16} />}
              component={Link}
              href="/settings"
              onClick={onClose}
              className="text-text-primary hover:bg-surface-hover"
            >
              Settings
            </Menu.Item>

            <Menu.Item
              leftSection={<IconMessageReport size={16} />}
              onClick={() => setFeedbackOpen(true)}
              className="text-text-primary hover:bg-surface-hover"
            >
              Share Feedback
            </Menu.Item>

            <Menu.Item
              leftSection={<GithubIcon className="h-4 w-4" />}
              component="a"
              href="https://github.com/positonic/ai-todo"
              target="_blank"
              rel="noopener noreferrer"
              className="text-text-primary hover:bg-surface-hover"
            >
              GitHub
            </Menu.Item>

            <Menu.Item
              leftSection={
                colorScheme === "dark" ? (
                  <IconSun size={16} />
                ) : (
                  <IconMoon size={16} />
                )
              }
              onClick={handleThemeToggle}
              className="text-text-primary hover:bg-surface-hover"
            >
              {colorScheme === "dark" ? "Light mode" : "Dark mode"}
            </Menu.Item>

            <Menu.Divider className="border-border-primary" />

            <Menu.Item
              leftSection={<IconLogout size={16} />}
              component={Link}
              href="/api/auth/signout"
              onClick={onClose}
              className="text-text-primary hover:bg-surface-hover"
            >
              Log out
            </Menu.Item>
          </Menu.Dropdown>
        </Menu>
      </div>

      <SidebarFeedback
        onClose={onClose}
        opened={feedbackOpen}
        onOpenChange={setFeedbackOpen}
      />
    </>
  );
}

function GithubIcon(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" {...props}>
      <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
    </svg>
  );
}
