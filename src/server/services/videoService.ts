...
async function generateBasicDescription(transcription: string): Promise<string> {
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
        ...
        body: JSON.stringify({
            ...
            messages: [
                {
                    role: "system",
                    content: "Create a structured markdown description of the crypto trading content. Focus on key points, trade setups, and market analysis."
                },
                {
                    role: "user",
    ...
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
        ...
        body: JSON.stringify({
            ...
            messages: [
                {
                    role: "system",
                    content: "Add appropriate timestamps to the markdown sections based on the provided captions. Each timestamp should link to the video section."
                },
                {
                    role: "user",
                    content: `
...
Please add appropriate [MM:SS](${videoUrl}&t=N) timestamp links to each section header based on when topics are first mentioned in the captions.
                    `
                }
            ],
            temperature: 0.7,
...
export async function summarizeTranscription(
    ...
    if (summaryType === 'description') {
        // First, generate the basic description without timestamps
        const basicDescription = await generateBasicDescription(transcription);
        
        // Then, if we have captions and videoUrl, add timestamps
        if (captions && videoUrl) {
            const descriptionWithTimestamps = await addTimestamps(basicDescription, captions, videoUrl);
            return { content: descriptionWithTimestamps };
        }
        
        return { content: basicDescription };
    }
