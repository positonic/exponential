...
describe("action router", () => {
  ...
  // ────────────────────────────────────────────────────────────────────
  // One2b agent integration: bulkCreateFromTranscript
  // ────────────────────────────────────────────────────────────────────
  describe("bulkCreateFromTranscript", () => {
    async function setupTranscript(opts?: { workspaceIdOverride?: string }) {
      const owner = await createUser(db);
      const ws = await createWorkspace(db, { ownerId: owner.id });
      const session = await db.transcriptionSession.create({
        data: {
          sessionId: `sess-${Math.random().toString(36).slice(2, 10)}`,
          workspaceId: opts?.workspaceIdOverride ?? ws.id,
          userId: owner.id,
          title: "Standup",
        },
      });
    ...
    it("creates actions for all items, resolves user assignee", async () => {
      ...
      expect(action.workspaceId).toBe(ws.id);
      expect(action.transcriptionSessionId).toBe(session.id);
      expect(action.sourceType).toBe("meeting");
      expect(action.sourceId).toBe(session.id);
      expect(action.lastUpdatedBy).toBe("AGENT");
      expect(action.assignees).toHaveLength(1);
      expect(action.assignees[0]!.user.id).toBe(member.id);
        ...
        items: [
          {
            ...
            assigneeEmail: "external@example.com",
            priority: "MEDIUM",
          },
        ],
      });
    ...
    it("auto-creates participant when email is unknown", async () => {
      ...
      const participant =
        await db.transcriptionSessionParticipant.findUnique({
          where: {
      ...
      expect(participant!.name).toBe("New Lead");
      expect(action.participantAssignees[0]!.participantId).toBe(participant!.id);
    });
    ...
    it("rejects mismatched transcript workspace", async () => {
      const owner = await createUser(db);
      const ws1 = await createWorkspace(db, { ownerId: owner.id });
      const ws2 = await createWorkspace(db, { ownerId: owner.id });
      const session = await db.transcriptionSession.create({
        data: {
          sessionId: `sess-${Math.random().toString(36).slice(2, 10)}`,
          workspaceId: ws2.id,
  ...
  // ────────────────────────────────────────────────────────────────────
  // One2b agent integration: findBySource
  // ────────────────────────────────────────────────────────────────────
  describe("findBySource", () => {
    ...
    it("filters by assigneeEmail when user is a workspace member", async () => {
      ...
      const a1 = await db.action.create({
        data: {
          name: "Mine",
          createdById: owner.id,
          workspaceId: ws.id,
      ...
      await db.actionAssignee.create({
        data: { actionId: a1.id, userId: member.id },
      });
      await db.action.create({
        data: {
          name: "Unassigned",
    ...
    it("filters by assigneeEmail when only a participant has that email", async () => {
      ...
      await db.actionParticipantAssignee.create({
        data: {
          actionId: a1.id,
      ...
      expect(result).toHaveLength(1);
      expect(result[0]!.name).toBe("External assignee");
    });
