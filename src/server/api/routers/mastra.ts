...
export const mastraRouter = createTRPCRouter({
  ...
  // These endpoints support the Fireflies configuration wizard
  // ============================================================================
  ...
  firefliesCheckExisting: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const personalIntegration = await ctx.db.integration.findFirst({
        where: {
          provider: "fireflies",
          userId: userId,
          teamId: null,
      ...
      if (personalIntegration) {
        return {
          exists: true,
          integrationId: personalIntegration.id,
          name: personalIntegration.name,
          createdAt: personalIntegration.createdAt.toISOString(),
          scope: "personal" as const,
        };
      }

      // Check for team integration if teamId provided
      if (input?.teamId) {
        const teamIntegration = await ctx.db.integration.findFirst({
          where: {
            provider: "fireflies",
            teamId: input.teamId,
          },
        ...
        if (teamIntegration) {
          return {
            exists: true,
            integrationId: teamIntegration.id,
            name: teamIntegration.name,
            createdAt: teamIntegration.createdAt.toISOString(),
            scope: "team" as const,
          };
        }
      }
  ...
  firefliesCreateIntegration: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!testResult.success) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: testResult.error ?? "Failed to verify Fireflies API key",
        });
      }

      // Verify team access if creating team integration
      if (input.scope === "team" && input.teamId) {
        ...
        if (!teamMember) {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "You must be a team owner or admin to create team integrations",
          });
        }
      }
  ...
  firefliesGenerateWebhookToken: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!integration) {
        throw new TRPCError({
          code: "NOT_FOUND",
          message: "Integration not found or you don't have access",
        });
      }

      // Parse expiration
      const expiresIn = input.expiresIn;
      let expiresMs = 90 * 24 * 60 * 60 * 1000; // 90 days default
      if (expiresIn.endsWith("d")) {
        expiresMs = parseInt(expiresIn) * 24 * 60 * 60 * 1000;
      } else if (expiresIn.endsWith("y")) {
        expiresMs = parseInt(expiresIn) * 365 * 24 * 60 * 60 * 1000;
      }
      ...
      // Store in VerificationToken (reusing existing table)
      await ctx.db.verificationToken.create({
        data: {
          identifier: `fireflies-webhook:${input.integrationId}`,
          token,
          expires: expiresAt,
        },
      });
  ...
  // Get the webhook URL for user's environment
  firefliesGetWebhookUrl: protectedProcedure.query(async () => {
    const baseUrl =
      process.env.NEXTAUTH_URL ??
