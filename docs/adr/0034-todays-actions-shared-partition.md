...
- **2026-09-09** — the scheduled **Daily summary** notification (`emit/summaries.ts`) had quietly carried a *third* "today" definition (created-by-me only, `dueDate` today only — no assignees, no `scheduledStart`). Its redesign converges it onto **Today's actions** via `partitionActions()`, cross-workspace like `/today`, with an overdue count instead of an overdue list. The Daily brief (voice) remains the one accepted divergence. See [ADR-0059](0059-daily-summary-structured-digest-per-channel-render.md).
- **2026-09-10** — the deferred convergence of voice is done. `get_todays_plan` no longer calls the due-only `generateBriefingData` (deleted, with `voice/dailyBrief.ts`); it now builds the ADR-0059 **Daily summary digest** (`buildDailySummary`) and renders it for speech (`voice/dailyContext.ts`), so voice, Matrix and email all speak **Today's actions** plus today's meetings, the overdue actions and the current cycle. The digest gained the overdue *names* (`overdueActions`, same `partitionActions` order) and bare ticket titles for that. The tool takes an optional `focus` (`meetings` / `actions` / `overdue` / `cycle`) so a follow-up reads one section in full, and a `timezone` from the device so "today" is the user's local day. The web morning briefing (`briefing.getMorningBriefing`) keeps its own due-only clauses; it is the last surface on that definition.
