...
The rule that voice endpoints (e.g. `voice.createSession`) accept **any** of three credentials, all resolving to one userId: NextAuth session cookie (web), `Authorization: Bearer <device-token>` (native app), or `x-api-key` (legacy/dev). Implemented in `apiKeyMiddleware` — Bearer/cookie are checked first via `ctx.session`, then the `x-api-key` `VerificationToken` lookup. Bearer wins when more than one is present.
