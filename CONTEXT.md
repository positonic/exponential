...
**Feature**:
A coherent slice of product capability inside a Product, stored as `Feature`. Has `status` (`IDEA`, `DEFINED`, `IN_PROGRESS`, `SHIPPED`, `ARCHIVED`), optional `vision`, optional alignment to a Goal (`Feature.goalId`). Groups Tickets (`Ticket.featureId`). A Feature has **no workspace of its own** — `Feature.productId` is its only container link, and its workspace is *derived through its Product* (`Product.workspaceId`). So "which workspace is this feature in" is always a Product question, never a direct field. See **Feature move**.
_Avoid_: Saying a Feature "belongs to a workspace" (it belongs to a Product); treating `goalId` as cross-workspace (a Feature aligns only to a Goal in its own Product's workspace).

**Feature move**:
Re-homing a Feature into a **Product in another Workspace** by re-pointing `Feature.productId`. Because a Feature has no workspace field, moving "to a workspace" is really moving "to a Product" — the user picks **workspace → Product** (two-step) via an explicit **Move** widget (a button + confirm modal, never an always-live inline dropdown — a move is a lossy cascade, not a field edit). The mover must be a non-viewer **member of both** the source and destination workspaces; the workspace picker lists only workspaces where the user is owner/admin/member, the Product picker only Products in the chosen workspace. The move is **one transaction** that carries the Feature's PRD body, scopes, user stories, and comments unchanged, **migrates its Tickets** with it, and performs a defined set of **lossy severances** (renumber tickets, drop ticket cycles, drop boundary-crossing ticket dependencies, clear non-member assignees, sever child Actions' `ticketId`, null the Feature's `goalId`, drop product-scoped `FeatureInsight` links, drop workspace-scoped tags). The confirm dialog must enumerate every loss before the user commits. See [ADR-0027](docs/adr/0027-cross-workspace-feature-move.md).
_Avoid_: "Reassign", "transfer" (use "move"); modelling it as editing a workspace field (there is none); a silent/inline move (it must be explicit and confirmed).
...
- **CRM Automations — PoC scope vs target model.** The target is "both": a typed Contact (**Customer type**) as the relationship spine, with `Deal`s tracked against it. The **PoC deliberately builds only the onboarding Automation** (typed contact → welcome email + **Agreement** for signature) and rides entirely on Contacts + the **Workflow** engine — it does **not** touch the deal board. **Deferred, not designed:** (1) multiple **Pipelines** per workspace / per-Customer-type boards (schema supports it; UI/`pipeline.get` assume one); (2) the drag-and-drop **Automation builder** UI (PoC automations are seeded in code); (3) engine **branching** (the Attio "Switch" node) — the engine is linear today; (4) deals-against-relationship. Don't model these until the PoC lands and the client confirms direction.
