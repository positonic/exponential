...
**PRD**:
A document-shaped spec: a Knowledge page linked to a **Feature** (optionally to a specific **Feature scope** for large extensions), arguing what should be built - problem and evidence (linked Insights), goals/non-goals, solution overview, scope cut, EARS **Requirements**, open questions, success metrics. A PRD is a moment-in-time artifact that ages into history; the Feature's own body stays the *living description* of what the capability currently is. One PRD per feature is the default; per-scope PRDs are allowed, never required.
