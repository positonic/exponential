...
- **CollapsibleSection**: chevron + uppercase label header for every detail-page/peek section. **Section anatomy rule:** the chevron owns the left edge; the component itself insets section content 16px (`inset` prop, default on; CRM surfaces opt out pending their own pass). Header-to-content spacing (8px), in-content group spacing (16px), and label-to-row spacing (4px) are three distinct steps - never equidistant.
- **Container list** (THE list grammar for linked items - actions, dependencies, scopes, requirements, docs): ONE bordered rounded container per group with 1px divider rows (`px-3 py-2/2.5`) - never per-row cards, never bare lines. The add affordance is frequency-based: a persistent input for high-frequency adds (actions, docs), a ghost "+ Add" reveal for rare ones (dependencies).
